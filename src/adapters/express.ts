...
import { SandboxOptions, SandboxRequest } from '../types';
...
export class ExpressAdapter {
   ...
   */
  private getCorsMiddleware(): RequestHandler | null {
   ...
   */
  private convertRequest(req: Request): SandboxRequest {
    ...
    return {
      ...
      raw: req as any as IncomingMessage
    };
  }
  ...
  middleware(): RequestHandler {
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      try {
        ...
        await this.sandbox.handleRequest(sandboxReq.raw, res as any);

      } catch (error) {
        if (this.options.errorHandler) {
          this.options.errorHandler(error as Error, req, res, next);
        } else {
 ...
 */
export async function createExpressAdapter(options: ExpressAdapterOptions): Promise<ExpressAdapter> {
 ...
 */
export async function createExpressMiddleware(options: ExpressAdapterOptions): Promise<RequestHandler> {
 ...
 */
export async function createExpressMiddlewares(options: ExpressAdapterOptions): Promise<RequestHandler[]> {
  const adapter = await createExpressAdapter(options);
