...
export class Router {
  ...
  private scenarios: ScenarioRule[] = [];
  ...
  private findMatchingScenarios(
    ...
    requestContext: { query: Record<string, string>; headers: Record<string, string> }
  ): ScenarioRule[] {
    const matchingScenarios: ScenarioRule[] = [];
  ...
  private valueMatches(actual: string | undefined, expected: string): boolean {
    if (actual === undefined) {
      return false;
    }
  ...
  static negotiateContentType(accepts: string[], produces: string[] = ['application/json']): string {
